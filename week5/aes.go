...
var rounds = 0
var keyLength = 0
var encryptedText [16]byte
var state [4][4]byte
var roundKey [240]byte
var Key = [32]byte{
	1, 2, 3, 4, 5, 6, 7, 8,
...
func getSboxInverse(num byte) byte {
	var rsbox = [256]byte{
...
func InverseSubBytes() {
	var i, j int
	for i = 0; i < 4; i++ {
		for j = 0; j < 4; j++ {
			state[i][j] = getSboxInverse(state[i][j])
		}
	}
}
